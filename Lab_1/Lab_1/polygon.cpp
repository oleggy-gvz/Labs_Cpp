...
CPolygon::CPolygon(int n, POINT ver, ...)
{
	POINT *v = &ver;
	SetAllVertexes(n, v);
}
...
{
	exist = CheckExist(vert, count); // провеяем треугольник на существование
	convex = CheckConvex(vert, count); // провеяем треугольник на выпуклость
}
...
int CPolygon::Load(std::ifstream &b_in)
{
	...
	try
	{
		...
		if (b_in.eof()) throw; // если была попытка прочитать за границы файла
